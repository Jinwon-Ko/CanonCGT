...
def evaluation(cfg, epoch, model, test_loader, pair_type='kk'):
    ...
    for i, batch in enumerate(test_loader):
        ...
        model_refer = batch['styler_refer'].cuda()
...
def analysis_kk(cfg, model, test_loader):
    model.eval()
    ...
    with torch.no_grad():
        ...
        print('Finish Style Transitions !')
        if cfg.eval:
            ...
            print(f'Eval on KK paired dataset ==> mPSNR {mPSNR:.5f}')
            logger(f'Eval on KK paired dataset ==> mPSNR : {mPSNR:.5f} dB \n', f'{cfg.save_dir}/stage3_results.txt')


def analysis_uu(cfg, model, test_loader):
    model.eval()
    ...
    mPSNR = psnr / n
    print(f'Eval on UU paired dataset ==> mPSNR {mPSNR:.5f}')
    logger(f'Eval on UU paired dataset ==> mPSNR : {mPSNR:.5f} dB \n', f'{cfg.save_dir}/stage3_results.txt')
