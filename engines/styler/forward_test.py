...
def evaluation(cfg, epoch, model, test_loader, best, now):
    ...
    for i, batch in enumerate(test_loader):
        ...
        n += len(imgs)
        psnr += eval_tools.measure_PSNR(outputs['restyled'], gts)
...
def analysis(cfg, model, test_loader):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(test_loader):
            ...
            # Evaluation
            PSNRs = eval_tools.measure_PSNR(outputs['restyled'], gts, reduction=None)
            ...
            if cfg.viz:
                ...
                for k in range(len(imgs)):
                    ...
                    viz_tools.update_image(outputs['restyled'][k], name=f'output_{style_idx:02d}')  # Model output
