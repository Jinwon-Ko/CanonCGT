...
def train_one_epoch(cfg, epoch, model, train_loader, criterion, optimizer):
    ...
    for i, batch in enumerate(train_loader):
        ...
        # Load data
        batch = batch_reformatting(batch)

        # Forward model
        outputs = {}
        outputs.update(model.get_style_centroids())
    ...
    logger("[Epoch %d Average Losses] %s\n" % (epoch, log), f'{cfg.save_dir}/stage2_losses_style_transition.txt')
    print('\n[Epoch %d Average Losses] %s' % (epoch, log))
    save_final_model(cfg, model, epoch)

    PSNR = psnr / num
    logger("[Epoch %d]  Train ==> PSNR %5f\n" % (epoch, PSNR), f'{cfg.save_dir}/stage2_train_performances.txt')
    print('[Epoch %d]  Train ==> PSNR %5f' % (epoch, PSNR))
    return model, optimizer, loss_t


def batch_reformatting(batch):
    reformat_batch = {}
    reformat_batch['src_style_label'] = batch['src_style_label'].cuda()
    reformat_batch['canonicalizer_input'] = batch['canonicalizer_input'].cuda()
    reformat_batch['canonicalizer_GT'] = batch['canonicalizer_GT'].cuda()
    reformat_batch['tgt_style_label'] = batch['tgt_style_label'].cuda()
    reformat_batch['restyler_refer'] = batch['restyler_refer'].cuda()
    reformat_batch['restyler_GT'] = batch['restyler_GT'].cuda()
    reformat_batch['canonical_refer'] = batch['canonical_refer'].cuda()
