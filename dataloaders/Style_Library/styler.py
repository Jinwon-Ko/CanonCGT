...
class Train_dataset(Dataset):
        ...
        for style_idx, style_name in self.style_dicts.items():
            ...
            for img_name in datalist:
                ...
                if tag not in tag_names:
                    style_img_path = os.path.join(img_dir, img_name)
                    canonical_path = os.path.join(canonical_dir, img_name)
                    self.samples.append((style_img_path, canonical_path, str(style_idx)))
    ...
    def __getitem__(self, idx):
        ...
        style_img_path, canonical_path, style_label = self.samples[idx]
        img = Image.open(canonical_path).convert("RGB")
        gt = Image.open(style_img_path).convert("RGB")
...
class Test_dataset(Dataset):
        ...
        for img_name in datalist:
            ...
            if tag in tag_names:
                ...
                for style_idx, style_name in self.style_dicts.items():
                    img_dir = os.path.join(root_dir, f'{style_idx}_{style_name}')
                    style_img_path = os.path.join(img_dir, img_name)
                    style_paths.append((style_img_path, str(style_idx)))
    ...
    def __getitem__(self, idx):
        ...
        img_tensor = self.transform(Image.open(canonical_path).convert("RGB"))
        gt_list = [self.transform(Image.open(style_img_path).convert("RGB")) for style_img_path, _ in style_paths]
